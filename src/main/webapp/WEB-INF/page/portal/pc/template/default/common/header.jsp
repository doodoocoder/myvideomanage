...
.yaoqishan-header .search{position: absolute;bottom: 20px;right: 0;width: 268px;height: 32px;padding: 2px 2px 2px 2px;background-color: #e5e9ef;background-color: rgba(0,0,0,.12);border-radius: 6px;font-size: 12px;z-index: 10;}
				...
				<c:forEach items="${navlist}" var="entity">
					<c:choose>
						<c:when test="${!empty active && fn:contains(entity.link, active)}">
							...
							<%--<li><a href="${pageContext.request.contextPath}/${entity.link}" >手术视频</a></li>
							<li><a href="${pageContext.request.contextPath}/${entity.link}" >直播</a></li>
							<li><a href="${pageContext.request.contextPath}/${entity.link}" >资源中心</a></li>--%>
						</c:when>
						<c:otherwise>
							<c:choose>
...
<script>
	...
	//切换样式
	/*$(".yaoqishan-nav .nav ul li").click(function () {
