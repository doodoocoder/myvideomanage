...
<script>
	...
	function getMediaLisByFilter(filterArr, pageNum) {
		...
		$.ajax({
			url : "${pageContext.request.contextPath}/api/get_media_list_by_filter.json",
			type : "POST",
			dataType : "json",
			traditional : "true",
			data : {
			...
			success : function(rtn) {
				...
				for (var i=0; i<list.length; i++) {
				    if(list[i].hasOwnProperty("videoInfo")){//被逻辑删除的是不能显示的
                        html += '<li>';
                        html += '<a href="play.action?videoId='+list[i].videoInfo.videoId+'" target="_blank" class="screenshot">';
                        ...
                        html += '</a>';
                        html += '<p class="screenshot-title"><a href="play.action?videoId='+list[i].videoInfo.videoId+'" target="_blank">'+list[i].biaoti+'</a></p>';
                        html += '</li>';
					}
