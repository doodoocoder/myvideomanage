package cn.shirdon.liao.service.video_info;
...
import cn.shirdon.liao.view.BizVideo;
...
public class VideoInfoService {
    ...
    public void batchDelete(String[] videoIdArr) throws QingException {
        for (int i = 0; i < videoIdArr.length; i++) {
            ...
            //执行本地删除
            VideoUtil.deleteLocal(videoInfo);
        }
     ...
     *
     */
    public List<VideoInfo> listNewVideo() {
        return iVideoInfoDAO.listNewVideo4Wx();
    }
    /**
     * 清空视频点击量
     *
    ...
    public List<BizVideo> listByMediaFieldValue(String field, String code) {
        List<BizVideo> list = iVideoInfoDAO.listByMediaFieldValue(field, code);
        String cd;
        String cnName;
        for (BizVideo b : list) {
            cd = b.getCode();
            cnName = SystemOptDictConfig.DICT.get(cd);
            b.setCnName(cnName);
        }
        return list;
    }
